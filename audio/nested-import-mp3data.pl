#!/usr/bin/perl -Wall
#
# nested-import-mp3data.pl
#
# Import audio files from a tree of nested sub-directories.
# Save MP3 Genre and Bitrate
#
# (C) Copyright 2015-2018 Fred Gleason <fredg@paravelsystems.com>
#
#  This requires the id3mtag package, available at
...
my $usage="USAGE: nested-import.pl <group-name> <filespec>";
if(scalar @ARGV lt 2) {
    print $usage."\n";
    exit 256;
}
...
foreach $file ($filespec) {
    find({wanted => \&ProcessDirectory,no_chdir => 1},$file);
}
...
sub ProcessDirectory
{
	...
	$cmd=$cmd."--set-string-user-defined=\"".$_."\" "."--set-string-client=\"".$genre."\" "."--set-string-agency=\"".$bitrate."\" ";
	$cmd=$cmd.$group_name." \"".$_."\"";
	if($title eq "<empty>") {
	    printf "Processing ".$_."\n";
	}
	else {
	    printf "Processing ".$_." [\"".$title."\"]\n";
	}
	system($cmd);
